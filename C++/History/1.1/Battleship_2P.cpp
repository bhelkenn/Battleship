...
public:
	...
	void populate() {
		...
		if (horizontal == false) {
			...
			else {cout << "Something went wrong in populate()" << endl;}
		}
		else {
	...
	void collect_taken(int num) {
		...
		//add the new values
		for (int i = 0; i < length; i++) {taken_coords[num + i] = ship[i];}
	}
	...
	bool is_valid(int num) {
		for (int i = 0; i < length; i++) {
			if (is_occupied(ship[i], num)) {return false;}
		}
		return true;
	}
...
Ship* create_ships() {
	...
	while (battleship.is_valid(count) != true) {battleship.populate();}
	...
	while (submarine.is_valid(count) != true) {submarine.populate();}
	...
	while (destroyer.is_valid(count) != true) {destroyer.populate();}
	...
	while (patrol_boat.is_valid(count) != true) {patrol_boat.populate();}
...
bool player_turn(Board ship_board, Board guess_board, string player_name, Ship *ships) {
	...
	cout << "\nNumber of ships remaining: " << remaining << endl;
	...
	cout << "Number of ships remaining: " << remaining << endl;
